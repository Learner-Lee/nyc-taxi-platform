...
![image-20260603161528584](STAGE_07_DWS层与查询优化.assets/image-20260603161528584.png)

**ADS 和 DWS 最容易混,区别全在那三个带 ★ 的字段。** `revenue_rank`(排名)、`revenue_wow`(环比)、`airport_rev_pct`(占比)——这三类是 ADS 独有的,DWS 里坚决不放。原因:它们都是**跨行 / 跨周期 / 跨字段**的计算(排名要比较所有区,环比要拿上周比,占比要拿机场÷总额)。DWS 只做"同一组内的直接聚合"(SUM/COUNT/AVG),把这些保留给最后一层。这样 DWS 才能被多个报表复用——换个报表要的排名口径不同,但底层的 `total_revenue` 是同一份。

**DWS 的字段 = 维度列 + 聚合列,粒度由 ADS 倒逼。** 看 DWS 卡:前三个 `stat_date / pu_borough / pu_zone` 是维度(GROUP BY 的列),后面全是聚合指标。粒度选"日×区域"不是随意的——因为 ADS 要按周、按区出报表,日和区是能往上卷的最细粒度;再细(到小时、到 zone)对这张周报是浪费,但运营那张小时热力图就需要,所以会另起一张 DWS。

**DWD 是地基,决定了"能不能建",上面三层只能用它有的原子字段。** 注意 DWD 里的 `pickup_date`、`pickup_hour`、`is_airport`、`pu_borough` 都标了"派生 / 维度退化"——这是 DWD 的活:把脏的原始时间戳清洗成可用的日期/小时,把机场判断固化成一个标志位,把维表的 borough/zone 提前 join 进来。正因为 DWD 把这些原子字段都备好了,上面 DWS 的每个聚合、ADS 的每个指标才有米下锅。**设计倒推到这一层会"撞墙":如果业务目标要的某个东西 DWD 里压根没有,那就得回头先在 DWD/ODS 把它造出来。** 这就是你说的"从 DWD 确定字段"的真正含义——DWD 是约束,也是源头。
...
看出来没?**三张表是同一批数据,越往上越少、越精炼。** 下面这张图就是这三张表的关系:

![image-20260603163501029](STAGE_07_DWS层与查询优化.assets/image-20260603163501029.png)

记住一个特别土的比方就够了:

- DWD = **你钱包里所有小票**,一张张乱塞,但啥都记着。
- DWS = 你**按天理一遍**,记成"这天一共花了多少"。
- ADS = 你发给爸妈的那句**"这个月周一花最多"**——已经是结论了。
